...
namespace Framework.Service
{
    public static class TestDataReader
    {
        static Configuration ConfigFile
        {
            get
            {
                string variableFromConsole = TestContext.Parameters.Get("env");
                string file = string.IsNullOrEmpty(variableFromConsole) ? "Identical" : variableFromConsole;
                int index = AppDomain.CurrentDomain.BaseDirectory.IndexOf("bin", StringComparison.Ordinal);

                ExeConfigurationFileMap configeMap = new ExeConfigurationFileMap
                {
